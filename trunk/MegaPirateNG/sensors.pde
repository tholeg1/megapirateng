...
// try the same as barometer filter for sonar


static long read_sonar(void)
{
 	long sonar_avgalt = 0;

	// add new data into our filter
	sonar_filter[sonar_filter_index] = sonar.read();
	sonar_filter_index++;

	// loop our filter
	if(sonar_filter_index >= SONAR_FILTER_SIZE)
		sonar_filter_index = 0;

	// zero out our accumulator

	// sum our filter
	for(byte i = 0; i < SONAR_FILTER_SIZE; i++){
		sonar_avgalt += sonar_filter[i];
	}


	// average our sampels
	return sonar_avgalt /= SONAR_FILTER_SIZE;
}
